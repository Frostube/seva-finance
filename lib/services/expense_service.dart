...
class ExpenseService with ChangeNotifier {
  ...
  Future<void> _loadExpenses() async {
    ...
    if (currentUserId == null) {
      debugPrint(
          'ExpenseService: User not authenticated. Loading expenses from local cache only.');
    ...
    try {
      debugPrint(
          'ExpenseService: User $currentUserId authenticated. Syncing expenses.');
      final snapshot = await _firestore
          ...
          .get();
      debugPrint(
          'ExpenseService: Fetched ${snapshot.docs.length} expenses from Firestore for user $currentUserId.');
      ...
      Map<String, Expense> localExpensesMap = {
        for (var e in _expenseBox.values) e.id: e
      };
      ...
      for (final expenseIdToDelete in expensesToDeleteLocally) {
        ...
        debugPrint(
            'ExpenseService: Deleted expense $expenseIdToDelete from local cache.');
      }

      _expenses = localExpensesMap.values.toList();
      _expenses.sort(
          (a, b) => b.date.compareTo(a.date)); // Keep expenses sorted by date
      debugPrint('ExpenseService: Synced ${_expenses.length} expenses.');
    } catch (e) {
      debugPrint(
          'Error syncing expenses with Firestore: $e. Using local cache as fallback.');
      _expenses = _expenseBox.values.toList();
  ...
  Future<List<Expense>> getExpensesForMonth(DateTime month) async {
    await initializationComplete;
    print(
        'ExpenseService: getExpensesForMonth called for month: $month. Internal _expenses count: ${_expenses.length}');
    ...
    print(
        'ExpenseService: getExpensesForMonth for $month returning ${filteredExpenses.length} items.');
  ...
  Future<List<CategoryMonthlySummary>> getExpensesByCategory(
    ...
    if (expensesForMonth.isEmpty) {
      print(
          'ExpenseService: No expenses found for $month. Returning empty list.');
      return [];
    }
    ...
    for (final expense in expensesForMonth) {
      ...
      categoryData[expense.categoryId]!['total'] =
          (categoryData[expense.categoryId]!['total'] as double) +
              expense.amount;

      if (expense.date.isAfter(
          categoryData[expense.categoryId]!['latestDate'] as DateTime)) {
        categoryData[expense.categoryId]!['latestDate'] = expense.date;
      }
    }

    final List<CategoryMonthlySummary> summaries =
        categoryData.entries.map((entry) {
      return CategoryMonthlySummary(
        categoryId: entry.key,
        categoryName: _categoryService.getCategoryNameById(entry.key,
    ...
    summaries.sort((a, b) {
      int dateCompare = b.latestTransactionDateInMonth
          .compareTo(a.latestTransactionDateInMonth);
      if (dateCompare != 0) {
        return dateCompare;
      }
    ...
    print(
        'ExpenseService: getExpensesByCategory for $month returning ${summaries.length} summaries, sorted.');
    return summaries;
  }

  Future<Map<String, dynamic>>
      getMonthlyExpenseSummaryForLastSixMonths() async {
    await initializationComplete;
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      print(
          'ExpenseService: Month: ${DateFormat('MMM yyyy').format(targetMonth)}, Total: $totalForMonth, SpotX: $spotX');
    }
    print(
        'ExpenseService: getMonthlyExpenseSummaryForLastSixMonths END - Spots: ${spots.length}, MaxSpending: $maxMonthlySpending');
    return {
      'spots': spots,
  ...
  Future<void> addExpense(Expense expense) async {
    ...
    try {
      ...
      debugPrint(
          'ExpenseService: Expense ${expense.id} added to Firestore for user $currentUserId');
      ...
      if (primaryWallet != null) {
        await _walletService.updateWalletBalance(
            primaryWallet.id, primaryWallet.balance - expense.amount);
      }
      _notificationService.addActionNotification(
        title: 'Expense Added',
        message:
            '${_categoryService.getCategoryNameById(expense.categoryId, defaultName: expense.categoryId)} expense of \$${expense.amount} added.',
        relatedId: expense.id,
      );

      // Check category budget alerts
      _checkCategoryBudgetAlerts(expense);
    } catch (e) {
  ...
  Future<void> deleteExpense(String expenseId) async {
    ...
    try {
      ...
      if (expenseIndex == -1) {
        debugPrint(
            'ExpenseService: Expense $expenseId not found in local list for deletion.');
      ...
      await _firestore
          ...
          .delete();
      debugPrint(
          'ExpenseService: Expense $expenseId deleted from Firestore for user $currentUserId');
      ...
      if (primaryWallet != null) {
        await _walletService.updateWalletBalance(
            primaryWallet.id, primaryWallet.balance + expenseToDelete.amount);
      }
      _notificationService.addActionNotification(
  ...
  Future<void> updateExpense(Expense expense) async {
    ...
    try {
      ...
      await _firestore
          ...
          .update(expenseData);
      debugPrint(
          'ExpenseService: Expense ${expense.id} updated in Firestore for user $currentUserId');

      final index = _expenses.indexWhere((e) => e.id == expense.id);
      if (index != -1) {
        _expenses[index] = expense;
        _expenses.sort(
            (a, b) => b.date.compareTo(a.date)); // Re-sort if date might change
        await _expenseBox.put(expense.id, expense);
      }
      _notificationService.addActionNotification(
        title: 'Expense Updated',
        message:
            '${_categoryService.getCategoryNameById(expense.categoryId, defaultName: expense.categoryId)} expense updated to \$${expense.amount}.',
        relatedId: expense.id,
      );
    } catch (e) {
  ...
  Future<Map<String, double>> getExpensesForPreviousMonth(
    ...
    final List<CategoryMonthlySummary> summaries =
        await getExpensesByCategory(previousMonth);
    ...
    if (walletBudget > 0) {
      ...
      if (totalSpent + amount > walletBudget) {
        _notificationService.addAlertNotification(
          ...
          title: 'Budget Alert',
          message:
              'This expense will exceed your monthly budget for the selected wallet.',
        );
      }
    }
  }
  ...
  Future<List<double>> getDailyExpensesForWeek(DateTime dateInThatWeek) async {
    ...
    DateTime mondayOfWeek =
        DateTime(dateInThatWeek.year, dateInThatWeek.month, dateInThatWeek.day)
            .subtract(Duration(days: daysToSubtract));
    print(
        'ExpenseService: getDailyExpensesForWeek - dateInThatWeek: $dateInThatWeek, mondayOfWeek: $mondayOfWeek');
    ...
    print(
        'ExpenseService: getDailyExpensesForWeek - dailyTotals (Mon-Sun): $dailyTotals');
    return dailyTotals;
  }
  ...
  Future<List<FlSpot>> getDailyExpenseSpotsForMonth(DateTime month) async {
    await initializationComplete;
    print(
        'ExpenseService: getDailyExpenseSpotsForMonth called for $month. Internal _expenses count: ${_expenses.length}');
    final List<FlSpot> spots = [];
    ...
    print(
        'ExpenseService: getDailyExpenseSpotsForMonth for $month returning ${spots.length} spots.');
    return spots;
  }
  ...
  Future<void> updateExpenseAndUpdateWallet(
    ...
    if (amountDifference != 0) {
      ...
      if (walletToUpdate != null) {
        await _walletService.updateWalletBalance(
            walletToUpdate.id, walletToUpdate.balance - amountDifference);
      }
    }
  }

  Future<Map<String, double>> getExpensesByCategoryForInsights(
      DateTime month) async {
    await initializationComplete;
    ...
    for (final expense in expensesForMonthList) {
      final categoryName = _categoryService.getCategoryNameById(
          expense.categoryId,
          defaultName: expense.categoryId);
      categoryTotals[categoryName] =
          (categoryTotals[categoryName] ?? 0.0) + expense.amount;
    }
    return categoryTotals;
  }

  /// Check category budget alerts when an expense is added
  void _checkCategoryBudgetAlerts(Expense expense) {
    // This will be implemented when CategoryBudgetService is available
    // For now, we'll just add a basic notification if the expense is large
    if (expense.amount > 100) {
      _notificationService.addAlertNotification(
        alertId: 'large_expense_${expense.id}',
