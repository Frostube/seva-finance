...
class CategoryService with ChangeNotifier {
  ...
  CategoryService(this._categoryBox, this._firestore, this._expenseBox) {
    // Modified constructor
    _initialLoadFuture = _loadCategories();

    // Listen to authentication changes to clear or reload categories
    _auth.authStateChanges().listen((User? user) {
      if (user != null) {
        debugPrint('CategoryService: User logged in, reloading categories...');
        _loadCategories();
      } else {
        debugPrint('CategoryService: User logged out, clearing categories...');
        _categories = [];
        notifyListeners();
      }
    });
  ...
  Future<void> _loadCategories() async {
    ...
    if (userId == null) {
      debugPrint(
          'CategoryService: User not authenticated. Loading categories from local cache only.');
    ...
    try {
      debugPrint(
          'CategoryService: User $userId authenticated. Starting Firestore sync.');
      final snapshot = await _firestore
          ...
          .get();
      debugPrint(
          'CategoryService: Fetched ${snapshot.docs.length} categories from Firestore for user $userId.');
      ...
      Map<String, ExpenseCategory> localCategoriesMap = {
        for (var cat in _categoryBox.values) cat.id: cat
      };
      ...
      for (final categoryIdToDelete in categoriesToDeleteLocally) {
        ...
        debugPrint(
            'CategoryService: Deleted category $categoryIdToDelete from local cache.');
      }
      ...
      debugPrint(
          'Error syncing categories with Firestore: $e. Using local cache.');
      _categories = _categoryBox.values.toList();
    }
  ...
  Future<ExpenseCategory> getOrCreateUncategorizedCategory() async {
    ...
    var uncategorized = _categories.firstWhere(
        (cat) => cat.id == uncategorizedId,
        orElse: () => ExpenseCategory(
            id: 'temp', name: 'temp', icon: 'temp')); // temp non-null for check
    if (uncategorized.id != 'temp') return uncategorized;

    // Check Hive next
    uncategorized = _categoryBox.get(uncategorizedId) ??
        ExpenseCategory(id: 'temp', name: 'temp', icon: 'temp');
    if (uncategorized.id != 'temp') {
      if (!_categories.any((cat) => cat.id == uncategorizedId))
    ...
    try {
      if (_userId != null) {
        await _firestore
            ...
            .doc(_userId)
            .collection('expenseCategories')
            .doc(newUncategorized.id)
            .set(newUncategorized.toJson());
      }
      await _categoryBox.put(newUncategorized.id, newUncategorized);
      if (!_categories.any((cat) => cat.id == newUncategorized.id))
        _categories.add(newUncategorized);
      debugPrint(
          'CategoryService: Created and saved "Uncategorized" category.');
    } catch (e) {
  ...
  Future<void> addCategory(ExpenseCategory category) async {
    ...
    if (category.id == uncategorizedId)
      throw Exception(
          'Cannot add a category with the reserved ID "uncategorized"');
    _isLoading = true;
    notifyListeners();
    try {
      await _firestore
          ...
          .set(category.toJson());
      debugPrint(
          'CategoryService: Added category ${category.id} to Firestore.');
  ...
  Future<void> updateCategory(ExpenseCategory category) async {
    ...
    if (category.id == uncategorizedId && category.name != uncategorizedName) {
      ...
      debugPrint(
          "CategoryService: Attempting to update Uncategorized category. Only icon/color changes are typically allowed.");
    }
    ...
    try {
      ...
      await _firestore
          ...
          .update(categoryData);
      debugPrint(
          'CategoryService: Updated category ${category.id} in Firestore.');
      ...
      if (index != -1) {
        ...
        _categories
            .add(category); // Should not happen if update is for existing
      }
      await _categoryBox.put(category.id, category);
    } catch (e) {
  ...
  Future<void> deleteCategory(String categoryId) async {
    if (_userId == null)
      throw Exception('User not authenticated to delete category');
    if (categoryId == uncategorizedId) {
      debugPrint(
          'CategoryService: Cannot delete the default "Uncategorized" category.');
      return;
    }
    ...
    try {
      final ExpenseCategory uncategorized =
      ...
      await batch.commit();
      debugPrint(
          'CategoryService: Updated ${expenseQuerySnapshot.docs.length} expenses in Firestore to category "${uncategorized.name}".');
      ...
      debugPrint(
          'CategoryService: Updated ${keysToUpdate.length} expenses in Hive to category "${uncategorized.name}".');
      ...
      await _firestore
          ...
          .delete();
      debugPrint(
          'CategoryService: Deleted category $categoryId from Firestore.');

      _categories.removeWhere((c) => c.id == categoryId);
      await _categoryBox.delete(categoryId);
      debugPrint(
          'CategoryService: Deleted category $categoryId from local stores.');
    } catch (e) {
  ...
  String getCategoryNameById(String categoryId,
    ...
    return category?.name ??
        (categoryId == uncategorizedId ? uncategorizedName : defaultName);
  }

  String getCategoryIconStringById(String categoryId,
      {String defaultIcon = 'help_outline'}) {
    final category = getCategoryById(categoryId);
    return category?.icon ??
        (categoryId == uncategorizedId ? uncategorizedIconName : defaultIcon);
  }
}
